...
namespace Viajes.Common.Services
{
    public class ApiService : IApiService
    {
        ...
        public async Task<Response> AddNewCost<T>(string urlBase, string servicePrefix, string controller, string tokenType, string accessToken, T CostTripRequest)
        {
            try
            {
                string request = JsonConvert.SerializeObject(CostTripRequest);
                StringContent content = new StringContent(request, Encoding.UTF8, "application/json");
                HttpClient client = new HttpClient
                {
                    BaseAddress = new Uri(urlBase)
